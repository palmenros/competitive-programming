...
void vueltaAtras(const int& n,const int& longCinta,const Datos datos,int& mochila1,int& mochila2,int& profundidad, int& solActual, int& satAcumulada)
{
	...
	if ((longCinta-mochila1) >= datos[profundidad].duracion)
	{
		...
		{
			vueltaAtras(n,longCinta,datos,mochila1,mochila2,profundidad,solActual,satAcumulada);
		}
		//Desmarcar
		profundidad--;
		mochila1 -= datos[profundidad].duracion;
