...
protected:
    ...
    bool inserta(T const& e, Link& a) {
        ...
        if (a == nullptr) { // se inserta el nuevo elemento e
            ...
            if (crece)
            {
                a->hijosIzq++; //Sumar 1 al número de elementos del hijo izquierdo
                reequilibraDer(a);
            }
        }
        else if (menor(a->elem, e)) {
    ...
    void rotaDer(Link& r2) {
        ...
        //Actualizar hijos izquierdos; únicamente actualizamos los de r2 que son los que cambian
    ...
    T borraMin(Link& a) {
        if (a->iz == nullptr) {
            ...
            T min = borraMin(a->iz);
            //Actualizar número de hijos izquierdos
            a->hijosIzq--;
            reequilibraIzq(a);
            return min;
        }
...
private:
    ...
    {
        //Obtener, suponiendo k correcto, el kesimo elemento del árbol formado por la raiz dada
        //Recordemos que el número hijosIzq de la raíz es su posición en el árbol
        //Si el igual a k, hemos terminado
        if (k == r->hijosIzq)
        {
            return r->elem;
        }
        //Si k es menor, buscar en el hijo izquierdo. En este caso el késimo del general es el késimo del izquierdo
        if (k < r->hijosIzq)
        {
            return kesimo(r->iz, k);
        }
        //Si es mayor buscar en el derecho, cuyo k-hijosIzq ésimo elemento es el k-ésimo del general
        if (k > r->hijosIzq)
        {
